...
export function getCode(lexcodes) {
...
export function error(ts) {
    throw ts;
}
export function errorResponse(response, ts) {
    if (!response.ok)
        throw { response, ts };
}
export function wrapError(code) {
    let name;
    let opt;
...
export function loadLexiPack(...file) {
    const pack = JSON.parse(readFileSync(join(...file)).toString());
    lexicon = pack;
    return pack;
}
